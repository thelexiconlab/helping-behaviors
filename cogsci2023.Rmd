...
geom_bar(stat = "identity", position = "dodge", width = 0.7, color = "gray24")+
  ...
  labs(y = "proportion of moves", title = "demonstrator moves")+
  theme_clean()+
...
first_move %>%
  ...
  summarise(ci = list(mean_cl_boot(value) %>% 
                        rename(mean=y, lwr=ymin, upr=ymax))) %>% unnest%>%
  ggplot(aes(x= goal_type, y = mean, group = rank, fill = rank)) +
...
gridExtra::grid.arrange(all_moves, useful_moves, nrow = 1)
```
...
better = probs  %>%
  rename(`baseline\ndemonstrator` = "literal_rank", 
         `pragmatic\ndemonstrator` = "prag_rank") %>%
  pivot_longer(names_to = "model", cols = c(`baseline\ndemonstrator`, `pragmatic\ndemonstrator`))%>%
    filter(utility == 1)
...
geom_bar(stat = "identity", position = "dodge", width = 0.7, color = "gray24")+
  ...
  labs(y = "mean rank", title = "demonstrator model ranks")+
  theme_clean()+
...
first_moves_only = m %>% select(ID, goal, goal_type, move_id, move, move_type) %>%
  ...
  mutate(move_id = ifelse(move_id == 0, "demonstrator", "helper")) %>%
...
first_moves_count = first_moves_only %>%
  group_by(ID, demonstrator, helper) %>%
  count()%>%
  group_by(demonstrator, helper) %>%
  summarize(mean = mean(n))

first_moves_count %>%
  ggplot(aes(x= demonstrator, y = mean, group = helper, fill = helper)) +
geom_bar(stat = "identity", position = "dodge", width = 0.7)+
  labs(y = "mean rank of first move", title = "all first moves", x = "")+
  theme_clean()+
  theme(aspect.ratio = 1)+
...
simulations %>% select(ID, goal, literal_utility, prag_utility) %>%
  ...
  mutate(model = fct_recode(model, `baseline\ndemonstrator` = "literal_utility",
                            `pragmatic\ndemonstrator`= "prag_utility")) %>%
...
geom_bar(stat = "identity", position = "dodge", width = 0.7, color = "gray24")+
  ...
  geom_errorbar(aes(ymin=per_lower, ymax=per_upr), size = 0.5, width=.15, 
                color = "black", position = position_dodge(0.70))+
  labs(y = "proportion of moves", title = "simulated first demonstrator moves", x = "")+
  theme_clean()+
  scale_fill_manual(values = c("darksalmon", "darkslategray4","darkolivegreen4"))+
  theme(plot.background = element_rect(
    color = "white"),
...
geom_bar(stat = "identity", position = "dodge", width = 0.7, color = "gray24")+
  ...
  geom_errorbar(aes(ymin=per_lower, ymax=per_upr), size = 0.5, width=.15, 
                color = "black", position = position_dodge(0.70))+
  labs(y = "proportion of moves", title = "simulated helper first moves", x = "")+
  theme_clean()+
  scale_fill_manual(values = c("darksalmon", "darkslategray4", "deeppink3","darkolivegreen4"))+
  theme(plot.background = element_rect(
    color = "white"),
